...
const TabItem = ({label, width, fontWeight, color}) => {
  return (
    <View style={[styles.smallFont, {width: width}]}>
      <Text style={[styles.tabText, {fontWeight: fontWeight, color: color}]}>
        {label}
      </Text>
    </View>
  );
  ...
  return (
    <View style={eksplore.font}>
      ...
      <ScrollView horizontal style={eksplore.horizontalFont}>
        ...
        <ExploreTab label="Recommended" availableWidth={130} fontWeight="bold" />
        <ExploreTab label="Popular" availableWidth={70} fontWeight="normal" />
        <ExploreTab label="Most Viewed" availableWidth={110} fontWeight="normal" />
        <ExploreTab label="Most Star" availableWidth={110} fontWeight="normal" />
      </ScrollView>
    </View>
  );
};
      ...
      <View style={akhir.image2}>
        {categories.map(category => (
          <View key={category.id} style={akhir.smallKategori}>
            ...
            <Text style={{ left: '22%', color: 'grey', fontWeight: '700', fontSize: 18 }}>
              {category.name}
            </Text>
          </View>
        ))}
